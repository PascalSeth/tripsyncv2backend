...
import { PurchaseConfirmationStatus } from "@prisma/client"
...
export class PurchaseConfirmationService {
   ...
   */
  async createPurchaseConfirmation(orderId: string) {
    try {
      // Get order details
      ...
      logger.error("Create purchase confirmation error:", error)
      throw error
    }
  }
  ...
  async confirmPurchase(confirmationToken: string, storeOwnerId: string) {
    try {
      ...
      const confirmation = await prisma.purchaseConfirmation.findUnique({
      ...
      if (confirmation.status !== PurchaseConfirmationStatus.PENDING) {
      ...
      await prisma.purchaseConfirmation.update({
   ...
   */
  async expireConfirmation(confirmationId: string) {
    try {
      const confirmation = await prisma.purchaseConfirmation.findUnique({
      ...
      if (!confirmation || confirmation.status !== PurchaseConfirmationStatus.PENDING) {
        return
      }
      ...
      await prisma.purchaseConfirmation.update({
        where: { id: confirmationId },
        data: {
          status: PurchaseConfirmationStatus.EXPIRED
        }
      })
      ...
      logger.info(`Purchase confirmation ${confirmationId} expired and refunded`)
    } catch (error) {
      logger.error("Expire confirmation error:", error)
      throw error
    }
  }
  ...
  private async processRefund(orderId: string) {
    try {
      ...
      if (!transaction) {
        logger.warn(`No payment transaction found for order ${orderId}`)
        return
      }
   ...
   */
  private async sendPurchaseConfirmationEmail(confirmation: any) {
    try {
      ...
      logger.info(`Purchase confirmation email sent to ${storeOwner.email}`)
    } catch (error) {
      logger.error("Send purchase confirmation email error:", error)
      throw error
    }
  }
   ...
   */
  private async sendPurchaseConfirmedEmail(confirmation: any) {
    try {
      const customer = confirmation.order.customer
      ...
      logger.info(`Purchase confirmed email sent to ${customer.email}`)
    } catch (error) {
      logger.error("Send purchase confirmed email error:", error)
      throw error
    }
  }
   ...
   */
  private async sendPurchaseExpiredEmail(confirmation: any) {
    try {
      const customer = confirmation.order.customer
      const store = confirmation.order.store
      ...
      await this.emailService.sendEmail(customer.email, subject, html)
      logger.info(`Purchase expired email sent to ${customer.email}`)
    } catch (error) {
      logger.error("Send purchase expired email error:", error)
      throw error
    }
  }
   ...
   * Schedule automatic expiration check
   */
  private scheduleExpirationCheck(confirmationId: string) {
    // Schedule the expiration check for 15 minutes from now
    setTimeout(async () => {
      try {
        await this.expireConfirmation(confirmationId)
      } catch (error) {
        logger.error(`Failed to expire confirmation ${confirmationId}:`, error)
      }
   ...
   */
  async getPurchaseConfirmation(token: string) {
    try {
      return await prisma.purchaseConfirmation.findUnique({
  ...
  async sendReminderEmails() {
    try {
      ...
      const pendingConfirmations = await prisma.purchaseConfirmation.findMany({
        where: {
          status: PurchaseConfirmationStatus.PENDING,
          expiresAt: {
            gt: new Date(),
      ...
      for (const confirmation of pendingConfirmations) {
        try {
          ...
          logger.error(`Failed to send reminder for confirmation ${confirmation.id}:`, error)
        }
      }

      logger.info(`Sent ${pendingConfirmations.length} reminder emails`)
    } catch (error) {
   ...
   * Send reminder email
   */
  private async sendReminderEmail(confirmation: any) {
    try {
      const storeOwner = confirmation.order.store.owner.user
      ...
      logger.info(`Reminder email sent to ${storeOwner.email} for confirmation ${confirmation.id}`)
    } catch (error) {
