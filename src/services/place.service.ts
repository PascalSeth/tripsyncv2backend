...
export class PlaceService {
    ...
    try {
      ...
      const placesWithStats = filteredPlaces.map((place) => {
        if (includeVotes && "placeVotes" in place) {
          ...
          const likes = place.placeVotes.filter((vote: any) => vote.isLiked).length
  ...
  async getPlaceById(placeId: string, userId?: string) {
    try {
      ...
      // Get vote statistics
      const voteStats = await prisma.placeVote.groupBy({
        by: ["isLiked"],
        where: { placeId },
        _count: true,
      })
      ...
      let userVote = null
      if (userId) {
        userVote = await prisma.placeVote.findFirst({
          where: {
            placeId,
            userId,
          },
  ...
  async createPlace(placeData: any, ownerId?: string) {
    try {
  ...
  async updatePlace(placeId: string, updateData: any, userId: string) {
    try {
  ...
  async submitVote(voteData: {
    ...
    try {
      ...
      if (existingVote) {
        ...
        return await prisma.placeVote.update({
        ...
        // Create new vote
        return await prisma.placeVote.create({
          data: {
            placeId: voteData.placeId,
            isLiked: voteData.isLiked,
  ...
  async searchPlaces(query: string, params: { page: number; limit: number }) {
    try {
  ...
  async getPlaceVotes(placeId: string) {
    try {
      ...
      const stats = await prisma.placeVote.groupBy({
        by: ["isLiked"],
        where: { placeId },
        _count: true,
      })
  ...
  async getCategorySuggestions(placeId: string) {
    try {
      const suggestions = await prisma.placeCategorySuggestion.findMany({
        where: { placeId },
        include: {
      ...
      const groupedSuggestions = suggestions.reduce((acc: any, suggestion) => {
