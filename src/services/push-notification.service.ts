...
export class WebSocketService {
  ...
  async notifyUser(userId: string, event: string, data: any): Promise<boolean> {
    try {
      ...
      logger.info(`User ${userId} is not online via WebSocket. Attempting to send email notification.`)
      ...
      logger.warn(`User ${userId} is offline and has no email to send fallback notification.`)
