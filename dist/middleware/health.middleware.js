...
async function healthCheck(req, res, next) {
    try {
        ...
        logger_1.default.info(`Health check completed in ${totalTime}ms - Status: ${overallStatus}`);
        if (overallStatus !== "healthy") {
            logger_1.default.warn("Health check issues detected:", {
...
async function checkGoogleMaps() {
    ...
    try {
        ...
        const response = await fetch(`https://maps.googleapis.com/maps/api/geocode/json?address=Lagos,Nigeria&key=${process.env.GOOGLE_MAPS_API_KEY}`);
        if (!response.ok) {
...
async function checkDatabasePerformance() {
    ...
    try {
        ...
        await database_1.default.$queryRaw `SELECT COUNT(*) FROM "User"`;
