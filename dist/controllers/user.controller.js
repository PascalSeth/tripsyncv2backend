    ...
    constructor() {
        ...
        this.updateEmail = async (req, res) => {
            try {
                ...
                const existingUserWithNewEmail = await database_1.default.user.findUnique({ where: { email: newEmail } });
                ...
                await this.notificationService.notifyCustomer(userId, {
                    type: client_1.NotificationType.ACCOUNT_UPDATE,
                    title: "Email Updated",
                ...
                logger_1.default.error("Update email error:", error);
                res.status(500).json({
                    success: false,
        ...
        this.updatePhoneNumber = async (req, res) => {
            try {
                const userId = req.user?.id;
                if (!userId) {
                    return res.status(401).json({ success: false, message: "Unauthorized" });
                ...
                const existingUserWithNewPhone = await database_1.default.user.findFirst({ where: { phone: newPhone } });
                ...
                logger_1.default.error("Update phone number error:", error);
                res.status(500).json({
                    success: false,
        ...
        this.enableTwoFactorAuth = async (req, res) => {
            try {
                const userId = req.user?.id;
                if (!userId) {
                    return res.status(401).json({ success: false, message: "Unauthorized" });
                ...
                logger_1.default.error("Enable 2FA error:", error);
                res.status(500).json({
                    success: false,
        ...
        // Verify 2FA setup
        this.verifyTwoFactorAuthSetup = async (req, res) => {
            try {
                const userId = req.user?.id;
                if (!userId) {
                    return res.status(401).json({ success: false, message: "Unauthorized" });
                ...
                logger_1.default.error("Verify 2FA setup error:", error);
                res.status(500).json({
                    success: false,
        ...
        this.disableTwoFactorAuth = async (req, res) => {
            try {
                const userId = req.user?.id;
                if (!userId) {
                    return res.status(401).json({ success: false, message: "Unauthorized" });
                ...
                logger_1.default.error("Disable 2FA error:", error);
                res.status(500).json({
                    success: false,
