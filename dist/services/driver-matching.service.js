...
class DriverMatchingService {
    ...
    async findNearbyDrivers(params) {
        try {
            ...
            const driversWithETA = await Promise.all(nearbyDrivers.map(async (driver) => {
                const eta = await this.locationService.calculateETA(driver.currentLatitude, driver.currentLongitude, latitude, longitude);
                return {
            ...
            driversWithETA.forEach((driver, index) => {
                console.log(`   ${index + 1}. ${driver.driverName} - ${(driver.distance / 1000).toFixed(2)}km away, ETA: ${driver.eta}min`);
            });
            return driversWithETA;
        }
        catch (error) {
            logger_1.default.error("Find nearby drivers error:", error);
            throw error;
        }
    }
    /**
    ...
    async findNearbyTaxiDrivers(params) {
        try {
            ...
            const availableTaxiDrivers = await database_1.default.taxiDriverProfile.findMany({
                where: {
                    isAvailable: true,
                    isOnline: true,
                    isVerified: true,
            ...
            const taxiDriversWithETA = await Promise.all(nearbyTaxiDrivers.map(async (driver) => {
                const eta = await this.locationService.calculateETA(driver.currentLatitude, driver.currentLongitude, latitude, longitude);
                return {
                    driverId: driver.user.id,
                    driverName: `${driver.user.firstName} ${driver.user.lastName}`,
            ...
            logger_1.default.error("Find nearby taxi drivers error:", error);
            throw error;
        }
    }
    /**
     ...
     */
    async notifyDriversAboutBooking(bookingId, driverIds, bookingData) {
        try {
            ...
            await database_1.default.driverNotification.createMany({
                data: notifications,
                skipDuplicates: true,
            });
            ...
            logger_1.default.error("Notify drivers about booking error:", error);
    ...
    async handleBookingTimeout(bookingId) {
        try {
            ...
            logger_1.default.error("Handle booking timeout error:", error);
        }
    }
    /**
     * Match driver to booking based on service type
     */
    async matchDriverToBooking(bookingId) {
        try {
            ...
            logger_1.default.error("Match driver to booking error:", error);
            throw error;
        }
    }
}
