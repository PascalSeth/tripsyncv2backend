...
class MovingService {
    ...
    async calculateMovingQuote(data) {
        try {
            // Calculate distance
            const distance = this.locationService.calculateDistance(data.pickupAddress.latitude, data.pickupAddress.longitude, data.dropoffAddress.latitude, data.dropoffAddress.longitude);
            ...
            const basePrice = basePrices[data.serviceTier] || basePrices.STANDARD;
            ...
            logger_1.default.error("Calculate moving quote error:", error);
            throw error;
        }
    }
    ...
    async acceptMovingJob(bookingId, moverId) {
        try {
            // Validate mover eligibility
            const moverProfile = await database_1.default.moverProfile.findUnique({
    ...
    async startMovingJob(bookingId, data) {
        try {
    ...
    async completeMovingJob(bookingId, data) {
        try {
            const booking = await database_1.default.booking.findFirst({
                where: {
                    id: bookingId,
    ...
    async getMovingBookings(userId, filters) {
        try {
    ...
    async getMoverEarnings(userId, params) {
        try {
            const moverProfile = await database_1.default.moverProfile.findFirst({
                where: { userId },
            });
            ...
            const where = { moverProfileId: moverProfile.id };
            ...
            const earnings = await database_1.default.moverEarning.findMany({
                where,
    ...
    async getMoverAnalytics(userId) {
        try {
            const moverProfile = await database_1.default.moverProfile.findFirst({
                where: { userId },
            });
            ...
            const monthlyEarnings = await database_1.default.moverEarning.findMany({
                where: {
                    moverProfileId: moverProfile.id,
    ...
    calculateMovingDuration(params) {
        ...
        baseDuration *= tierMultipliers[params.serviceTier] || 1.0;
        // Additional services
    ...
    determineCrewSize(serviceTier, itemCount) {
        const baseCrew = {
            BASIC: 2,
            STANDARD: 3,
            PREMIUM: 4,
        };
        let crewSize = baseCrew[serviceTier] || 2;
    ...
    determineTruckSize(estimatedVolume, itemCount) {
        if (estimatedVolume) {
            if (estimatedVolume <= 10)
                return "SMALL_VAN";
            if (estimatedVolume <= 25)
                return "MEDIUM_TRUCK";
