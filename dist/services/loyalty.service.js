...
class LoyaltyService {
    constructor() {
        ...
        this.POINTS_EXPIRY_DAYS = 365; // Points expire after 1 year
     ...
     */
    async awardBookingPoints(bookingId) {
        try {
            ...
            const updatedProfile = await database_1.default.customerProfile.update({
                where: { userId },
                data: {
                    loyaltyPoints: {
            ...
            logger_1.default.error("Award booking points error:", error);
            throw error;
        }
    }
    /**
    ...
    async getLoyaltyStatus(userId) {
        try {
            ...
            logger_1.default.error("Get loyalty status error:", error);
            throw error;
        }
    }
    /**
    ...
    async getAvailableRewards(userId) {
        try {
            ...
            const rewards = this.loyaltyRewards.filter((reward) => reward.isActive &&
            ...
            logger_1.default.error("Get available rewards error:", error);
            throw error;
        }
    }
    /**
    ...
    async redeemReward(userId, rewardId) {
        try {
            ...
            if (reward.usageLimit) {
                const usageCount = this.loyaltyTransactions.filter((t) => t.userId === userId && t.rewardId === rewardId && t.type === "SPENT").length;
                if (usageCount >= reward.usageLimit) {
                    throw new Error("Usage limit exceeded");
                }
            }
            // Generate reward code
            const rewardCode = this.generateRewardCode(reward.category);
            ...
            logger_1.default.error("Redeem reward error:", error);
            throw error;
        }
    }
    /**
    ...
    async getLoyaltyAnalytics(dateRange) {
        try {
            ...
            const [profiles] = await Promise.all([
                database_1.default.customerProfile.findMany({
            ...
            const averagePointsPer = profiles.reduce((sum, p) => sum + p.loyaltyPoints, 0);
            const averagePointsPerUser = activeUsers > 0 ? averagePointsPer / activeUsers : 0;
            // Calculate expiry rate (simplified)
            const expiredTransactions = transactions.filter((t) => t.expiresAt && t.expiresAt < new Date() && t.type === "EARNED");
            ...
            logger_1.default.error("Get loyalty analytics error:", error);
            throw error;
        }
    }
    /**
     ...
     */
    async processTierUpgrade(userId, oldTier, newTier) {
        try {
            ...
            logger_1.default.error("Process tier upgrade error:", error);
        }
    }
    calculateTier(points) {
        if (points >= this.TIER_THRESHOLDS.DIAMOND)
            return "DIAMOND";
        if (points >= this.TIER_THRESHOLDS.PLATINUM)
            return "PLATINUM";
        if (points >= this.TIER_THRESHOLDS.GOLD)
            return "GOLD";
        if (points >= this.TIER_THRESHOLDS.SILVER)
            return "SILVER";
        return "BRONZE";
    }
    getNextTier(currentTier) {
        const tiers = Object.keys(this.TIER_THRESHOLDS);
    ...
    async calculateServiceBonus(serviceType, basePoints) {
    ...
    async calculateStreakBonus(userId, basePoints) {
        try {
            ...
            if (streak >= 7)
                return Math.floor(basePoints * 0.5); // 50% bonus for 7+ day streak
            if (streak >= 3)
                return Math.floor(basePoints * 0.2); // 20% bonus for 3+ day streak
            return 0;
        }
        catch (error) {
            logger_1.default.error("Calculate streak bonus error:", error);
            return 0;
        }
    }
    async calculateTimeBonus(bookingTime, basePoints) {
    ...
    calculateConsecutiveDays(dates) {
        if (dates.length === 0)
            return 0;
        const uniqueDays = Array.from(new Set(dates.map((date) => date.toISOString().split("T")[0]))).sort((a, b) => new Date(b).getTime() - new Date(a).getTime());
        let streak = 1;
    ...
    async getUserTier(userId) {
        try {
            const profile = await database_1.default.customerProfile.findUnique({
                where: { userId },
            });
            if (!profile)
                return "BRONZE";
            ...
            logger_1.default.error("Get user tier error:", error);
            return "BRONZE";
        }
    }
    async addLoyaltyTransaction(transaction) {
