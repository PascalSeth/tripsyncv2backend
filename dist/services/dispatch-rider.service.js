...
exports.DispatchRiderService = void 0;
...
class DispatchRiderService {
    ...
    async onboardDispatchRider(userId, onboardingData) {
        try {
            ...
            const existingProfile = await database_1.default.deliveryProfile.findUnique({
                where: { userId },
            });
            if (existingProfile) {
            ...
            const deliveryProfile = await database_1.default.deliveryProfile.create({
                data: {
                    userId,
    ...
    async updateProfile(userId, updateData) {
        try {
            const deliveryProfile = await database_1.default.deliveryProfile.findUnique({
                where: { userId },
            });
            if (!deliveryProfile) {
                throw new Error("Delivery profile not found");
            }
            const updatedProfile = await database_1.default.deliveryProfile.update({
                where: { userId },
                data: {
            ...
            logger_1.default.error("Update delivery profile error:", error);
            throw error;
        }
    }
    async getProfile(userId) {
        try {
            const profile = await database_1.default.deliveryProfile.findUnique({
                where: { userId },
                include: {
                    user: {
                        select: {
            ...
            logger_1.default.error("Get delivery profile error:", error);
            throw error;
        }
    }
    async addVehicle(userId, vehicleData) {
        try {
            ...
            logger_1.default.error("Add delivery vehicle error:", error);
            throw error;
        }
    }
    async updateVehicle(userId, vehicleId, updateData) {
        try {
            ...
            logger_1.default.error("Update delivery vehicle error:", error);
            throw error;
        }
    }
    async getVehicles(userId) {
        try {
            ...
            logger_1.default.error("Get delivery vehicles error:", error);
            throw error;
        }
    }
    async uploadDocument(userId, documentData) {
        try {
            ...
            const document = await database_1.default.deliveryDocument.create({
                data: {
            ...
            logger_1.default.error("Upload delivery document error:", error);
            throw error;
        }
    }
    async getDocuments(userId) {
        try {
            ...
            logger_1.default.error("Get delivery documents error:", error);
            throw error;
        }
    }
    async updateAvailability(userId, availabilityData) {
        try {
            const profile = await database_1.default.deliveryProfile.update({
                where: { userId },
                data: {
            ...
            logger_1.default.error("Update delivery availability error:", error);
            throw error;
        }
    }
    async updateLocation(userId, locationData) {
        try {
            ...
            logger_1.default.error("Update delivery location error:", error);
            throw error;
        }
    }
    async getDeliveryRequests(userId, filters) {
        try {
            ...
            const [deliveryRequests, total] = await Promise.all([
                database_1.default.delivery.findMany({
                    where,
                    include: {
                        order: {
                            include: {
            ...
            if (deliveryProfile.currentLatitude && deliveryProfile.currentLongitude) {
                filteredRequests = deliveryRequests.filter((request) => {
                    const distance = this.locationService.calculateDistance(deliveryProfile.currentLatitude, deliveryProfile.currentLongitude, request.pickupLocation.latitude, request.pickupLocation.longitude);
    ...
    async acceptDeliveryRequest(dispatchRiderId, deliveryId) {
        try {
            ...
            // Check distance
            if (deliveryProfile.currentLatitude && deliveryProfile.currentLongitude) {
                const distance = this.locationService.calculateDistance(deliveryProfile.currentLatitude, deliveryProfile.currentLongitude, delivery.pickupLocation.latitude, delivery.pickupLocation.longitude);
                if (distance > (deliveryProfile.maxDeliveryDistance || 50000)) {
                    throw new Error("Delivery location is outside your delivery range");
                }
            }
            // Update delivery
            const updatedDelivery = await database_1.default.delivery.update({
                where: { id: deliveryId },
                data: {
                    dispatchRiderId,
                    status: "ASSIGNED",
            ...
            const eta = await this.calculateETA(deliveryProfile.currentLatitude, deliveryProfile.currentLongitude, delivery.pickupLocation.latitude, delivery.pickupLocation.longitude);
            // Notify store owner
            if (delivery.order.store?.ownerId) {
                await this.notificationService.notifyCustomer(delivery.order.store.ownerId, {
                    type: client_1.NotificationType.DISPATCH_RIDER_ASSIGNED,
    ...
    async rejectDeliveryRequest(dispatchRiderId, deliveryId, reason) {
        try {
            // Log rejection for analytics
            await database_1.default.deliveryRejection.create({
                data: {
                    deliveryId,
                    dispatchRiderId,
    ...
    async startPickup(dispatchRiderId, deliveryId) {
        try {
            const delivery = await database_1.default.delivery.findFirst({
                where: {
                    id: deliveryId,
                    dispatchRiderId,
                    status: "ASSIGNED",
            ...
            // Add tracking update
            await database_1.default.deliveryTracking.create({
                data: {
                    deliveryId,
            ...
            if (delivery.order.store?.ownerId) {
                await this.notificationService.notifyCustomer(delivery.order.store.ownerId, {
                    type: client_1.NotificationType.PICKUP_STARTED,
    ...
    async confirmPickup(dispatchRiderId, deliveryId, confirmationData) {
        try {
            const delivery = await database_1.default.delivery.findFirst({
                where: {
                    id: deliveryId,
    ...
    async startDelivery(dispatchRiderId, deliveryId) {
        try {
            const delivery = await database_1.default.delivery.findFirst({
                where: {
                    id: deliveryId,
            ...
            // Notify customer
            await this.notificationService.notifyCustomer(delivery.order.customerId, {
                type: client_1.NotificationType.DELIVERY_STARTED,
    ...
    async completeDelivery(dispatchRiderId, deliveryId, completionData) {
        try {
            const delivery = await database_1.default.delivery.findFirst({
                where: {
                    id: deliveryId,
    ...
    async reportIssue(dispatchRiderId, deliveryId, issueData) {
        try {
            const issue = await database_1.default.deliveryIssue.create({
                data: {
                    deliveryId,
                    dispatchRiderId,
            ...
            await this.notificationService.notifyAdmins({
                type: client_1.NotificationType.DELIVERY_ISSUE_REPORTED,
                ...
                priority: issueData.severity === "HIGH" ? client_1.PriorityLevel.URGENT : client_1.PriorityLevel.STANDARD,
            });
            return issue;
        }
        catch (error) {
    ...
    async getDispatchRiderDeliveries(dispatchRiderId, filters) {
        try {
            const { status, page = 1, limit = 20, dateFrom, dateTo } = filters;
            const skip = (page - 1) * limit;
            const where = { dispatchRiderId };
            if (status)
                where.status = status;
            if (dateFrom || dateTo) {
                where.createdAt = {};
            ...
            logger_1.default.error("Get dispatch rider deliveries error:", error);
            throw error;
        }
    }
    async getDispatchRiderEarnings(dispatchRiderId, filters) {
        try {
            ...
            const earnings = await database_1.default.deliveryEarning.findMany({
                where: {
                    deliveryProfileId: deliveryProfile.id,
                    date: {
                        gte: filterStartDate,
            ...
            logger_1.default.error("Get dispatch rider earnings error:", error);
            throw error;
        }
    }
    async getDispatchRiderAnalytics(dispatchRiderId) {
        try {
            const deliveryProfile = await database_1.default.deliveryProfile.findFirst({
                where: { userId: dispatchRiderId },
            });
            ...
            const [monthlyStats, weeklyStats, recentDeliveries] = await Promise.all([
                database_1.default.deliveryEarning.aggregate({
                    where: {
                        deliveryProfileId: deliveryProfile.id,
            ...
            logger_1.default.error("Get dispatch rider analytics error:", error);
            throw error;
        }
    }
    async getAllDispatchRiders(filters) {
        try {
            ...
            const [dispatchRiders, total] = await Promise.all([
                database_1.default.deliveryProfile.findMany({
                    where,
                    include: {
                        user: {
                            select: {
                ...
                database_1.default.deliveryProfile.count({ where }),
            ]);
            return {
                dispatchRiders,
                pagination: {
    ...
    async verifyDispatchRider(dispatchRiderId) {
        try {
            const dispatchRider = await database_1.default.deliveryProfile.update({
                where: { userId: dispatchRiderId },
                data: {
                    isVerified: true,
    ...
    async suspendDispatchRider(dispatchRiderId, reason) {
        try {
            const dispatchRider = await database_1.default.deliveryProfile.update({
                where: { userId: dispatchRiderId },
                data: {
            ...
            await this.notificationService.notifyCustomer(dispatchRiderId, {
                type: client_1.NotificationType.ACCOUNT_SUSPENDED,
                title: "Account Suspended",
                body: `Your dispatch rider account has been suspended. Reason: ${reason}`,
                priority: client_1.PriorityLevel.CRITICAL,
            });
            return dispatchRider;
        }
        catch (error) {
    ...
    async markOrderReady(orderId, storeOwnerId, orderData) {
        try {
    ...
    async getStoreOrders(storeId, storeOwnerId, filters) {
        try {
            // Verify store ownership
    ...
    async updateDeliveryTracking(deliveryId, dispatchRiderId, trackingData) {
        try {
            ...
            try {
                ...
                logger_1.default.warn("Failed to broadcast location updates:", socketError);
            }
        }
        catch (error) {
    ...
    async startDeliveryTracking(deliveryId, dispatchRiderId) {
        try {
            await database_1.default.deliveryTracking.create({
                data: {
                    deliveryId,
            ...
            logger_1.default.error("Start delivery tracking error:", error);
        }
    }
    async notifyNearbyDispatchRiders(delivery) {
        return this.findAndNotifyDispatchRiders(delivery);
    }
    async findAndNotifyDispatchRiders(delivery) {
        try {
            // Find nearby available dispatch riders
            const nearbyRiders = await database_1.default.deliveryProfile.findMany({
                where: {
                    isAvailable: true,
                    isOnline: true,
                    isVerified: true,
            ...
            const pickupLocation = await database_1.default.location.findUnique({
            ...
            const ridersWithDistance = nearbyRiders
                .map((rider) => {
                const distance = this.locationService.calculateDistance(rider.currentLatitude, rider.currentLongitude, pickupLocation.latitude, pickupLocation.longitude);
            ...
            for (const rider of ridersWithDistance) {
                await this.notificationService.notifyCustomer(rider.user.id, {
                    type: client_1.NotificationType.NEW_DELIVERY_REQUEST,
            ...
            logger_1.default.error("Find and notify dispatch riders error:", error);
        }
    }
    async findAndNotifyNextDispatchRider(delivery) {
        try {
            // Implementation for finding next available dispatch rider
            logger_1.default.info(`Finding next dispatch rider for delivery ${delivery.id}`);
            // This would use similar logic to findAndNotifyDispatchRiders
        }
        catch (error) {
            logger_1.default.error("Find next dispatch rider error:", error);
        }
    }
    getWeekStart(date) {
    ...
    async getPendingDeliveryRequests(dispatchRiderId) {
        try {
            const deliveryProfile = await database_1.default.deliveryProfile.findUnique({
                where: { userId: dispatchRiderId },
            });
            ...
            const pendingRequests = await database_1.default.delivery.findMany({
                where: {
                    status: "PENDING",
    ...
    async getActiveDelivery(dispatchRiderId) {
        try {
            const activeDelivery = await database_1.default.delivery.findFirst({
                where: {
                    dispatchRiderId,
                    status: {
    ...
    async getActiveDeliveries(dispatchRiderId) {
        try {
            const activeDeliveries = await database_1.default.delivery.findMany({
                where: {
                    dispatchRiderId,
                    status: {
                        in: ["ASSIGNED", "PICKUP_IN_PROGRESS", "PICKED_UP", "IN_TRANSIT"],
            ...
            logger_1.default.error("Get active deliveries error:", error);
            throw error;
        }
    }
}
exports.DispatchRiderService = DispatchRiderService;
